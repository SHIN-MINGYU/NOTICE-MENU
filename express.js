...
app.get('/',function(req,res){//home main
    db.query('SELECT * FROM notice ORDER BY notice_id desc',function(err,notice){
...
app.post('/create_notice',function(req,res){ //게시판 생성 과정
    ...
    req.on('end', function(){
        ...
        db.query(`INSERT INTO notice (notice_id, title, name, date, sympathy, content,password)  
                 VALUES ("${notice_id}","${title}","${name}","${NOW}",${0},"${content}","${password}")`,
                function(err,result){
                    if(err){
                        throw err;
                    }
